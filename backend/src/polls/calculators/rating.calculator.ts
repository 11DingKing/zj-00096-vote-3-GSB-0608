...
@Injectable()
export class RatingCalculator implements ResultCalculator {
  constructor(
  ...
  async calculate(pollId: number, options: Option[]): Promise<OptionResult[]> {
    const rawResults = await this.votesRepository
      ...
      .addSelect(
        "COALESCE(AVG(CASE WHEN vote.ratingValue IS NOT NULL AND vote.ratingValue <> 0 THEN vote.ratingValue END), 0)",
        "avgRating",
      )
    ...
    const dataMap = new Map<number, { count: number; avgRating: number }>();
    let totalVotes = 0;
    for (const row of rawResults) {
      const count = parseInt(row.count, 10) || 0;
      const avgRating = parseFloat(row.avgRating) || 0;
      dataMap.set(parseInt(row.optionId, 10), { count, avgRating });
      totalVotes += count;
    }

    const results: OptionResult[] = options.map((option) => {
      const data = dataMap.get(option.id) || { count: 0, avgRating: 0 };
